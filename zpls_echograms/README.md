...
* In the case of the global moorings, the OOI 8-letter site name is appended with either an "_Upper" or "_Lower" to 
  indicate which of the sensors is being used (there are two on the 200 m sphere, one looking up towards the surface,
...
For the AZFP, the data is organized by the instrument in a folder called `DATA/`, below which the files are organized
in folders named according the year and month (e.g. `201610`). The files are named according to the year, month, day
and hour the data was recorded (e.g. `16101401.01A`). The higher level directory structure above `DATA/` is created by 
OOI technicians. If the ZPLS data from the [Global Argentine Basin, upward looking sensor for Deployment 
3](https://rawdata.oceanobservatories.org/files/GA02HYPM/R00003/instruments/ZPLSG_sn55067/) was downloaded to a local 
...
For the EK60, the data is organized by OOI technicians in folders based on the year, month and day (e.g. `2017/01/01`).
...
from 2017 was downloaded to local folder named `CE04`, the folders would look like:
...
Processed data is created in a folder of the users choice. We recommend placing the data at the same level as the root 
data directory in a separate folder called `processed`. The data in the `processed` folder is organized into subfolders 
...
the three data types list above. The files share the same naming convention as the processed and averaged data (e.g. 
...
# tilt angle correction (15 degrees for GA02HYPM_Upper) and the theoretical 
...
If the instrument is deployed at 160 m, instead of the theoretical depth of 150 m. The optional flag `-dd` 160 will 
overwrite the mooring configuration.
